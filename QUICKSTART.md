...
**Audio Module:**
```
ESP32-C6 GPIO19 (TX) → MP3 Module RX
ESP32-C6 GPIO20 (RX) → MP3 Module TX
...
3. Add MP3 files:
   - `01/001.mp3` to `01/018.mp3` (emotion sounds)
   - `02/001.mp3` and `02/002.mp3` (tap sounds)
   - `03/001.mp3` (sleep sound)
4. Insert SD card into MP3 module

**Quick tip**: Start with just a few files to test, add more later!
...
2. **Wait 15s**: Emotion should change randomly
3. **Shake**: Should trigger angry emotion with sound
4. **Single tap**: Should play tap sound
5. **Wait 30s**: Should show sleepy eyes
6. **Wait 60s**: Should enter deep sleep
7. **Shake again**: Should wake up!
